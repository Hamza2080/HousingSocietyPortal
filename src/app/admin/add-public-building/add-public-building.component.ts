...
@Component({
  selector: 'app-add-public-building',
  templateUrl: './add-public-building.component.html',
  styleUrls: ['./add-public-building.component.css']
})
export class AddPublicBuildingComponent implements OnInit {
  ...
  onSubmit() {
    this.isLoading = true;
      ...
      this.adminService.addPublicBuilding(this.payload).then(res => {
        console.log(res);
        this.isLoading = false;
        this.relatedModal.close(true);
      }).catch(err => {
        console.log(err);
        this.toastr.error("There is some error while adding publicBuilding " + err.Message || err);
        this.isLoading = false;
      })
  }
